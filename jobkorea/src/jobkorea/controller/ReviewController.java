...
public class ReviewController {
   ...
   // 후기 수정
   public boolean rUpdate(ReviewDto reviewDto, int rno, ArrayList<Integer> array) {
      boolean state = false;
      for(int index = 0; index <= array.size()-1; index++) {
         int ar = array.get(index);
         if(ar == rno) {
   ...
   // 후기 삭제
   public boolean rDelete(ReviewDto reviewDto, ArrayList<Integer> array) {
      boolean state = false;
      
      for(int index = 0; index <= array.size()-1; index++) {
         int ar = array.get(index);
         if(ar == reviewDto.getRno()) {
            state = true;
         } // if end
      } // for end
      if(state == false) {
